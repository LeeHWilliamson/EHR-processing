...
'''
inputs
true_positives: the number of agent results that also appear in ground truth
agent_actual: the agent's returned result
ground_truth: the ground truth for this task
output
precision as int
'''
def calc_precision(true_positives: int, agent_actual: list[str], ground_truth: list[str]):
    if agent_actual: #avoid division by zero
        precision = true_positives / len(agent_actual)
