...
## Motivation
This application is designed to address 2 major issues that providers face when designing governance policy surrounding AI and determining the efficacy of existing AI tools.
### Assessing AI performance in nuanced clinical tasks
Modern AI tools show great potential in their ability to carry out clinical tasks that go far beyond basic data retrieval, tasks such as summarizing patient medical history, interpreting imaging results, and supporting accurate diagnosis. Evaluating the efficacy of AI tools for certain tasks, however, can be subjective, time-intensive, and require expert feedback. These complications largely stem from the fact that patient health records are dynamic, ever-growing entities. New information is being added to these records all the time, and determining the relevance of any specific information to the current clinical task often requires expert domain knowledge and a firm grasp of a patient's medical history.
There is no shortage of AI tools that demonstrate great domain knowledge in the medical field, but clearly capturing all details of a patient's medical history as they relate to a specific task presents an entirely separate challenge for the agents. Performance in this domain can vary greatly between agents, tasks, database layouts, and even among specific patient charts. Even with the most advanced AI agents, medical providers are left with lingering questions; how will this agent perform on my specific workflows, within my electronic medical record, and on my patients? And an even more fundamental question, how can we answer these questions without investing significant resources in the wrong AI agent, and without exposing protected patient health information?
in their electronic medical record setup, and on their patients. Furthermore, they 
Performance can vary by agent, task, and by the exact layout of data contained in an provider's servers. Meanwhile, the medical providers are often limited in the options they have for determining whether any particular AI agent will be successful in their specific context without exposing protected patient data to external AI agents.
